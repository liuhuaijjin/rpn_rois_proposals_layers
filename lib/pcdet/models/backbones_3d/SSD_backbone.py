...
class SSDBackbone(nn.Module):
    def __init__(self, model_cfg, input_channels, **kwargs):
        ...
        for k in range(self.model_cfg.SA_CONFIG.NPOINTS.__len__()):
            ...
            if self.layer_types[k] == 'SA_Layer':
                ...
                self.SA_modules.append(pointnet2_modules.Vote_layer(mlp_list=self.model_cfg.SA_CONFIG.MLPS[k], #输出128
                                                                    pre_channel=channel_out_list[self.layer_inputs[k]], #输入256
                                                                    max_translate_range=self.max_translate_range)) #[-3.0,-2.0,-3.0]
                print(self.model_cfg.SA_CONFIG.MLPS[k],channel_out_list[self.layer_inputs[k]],self.max_translate_range)
            channel_out_list.append(self.model_cfg.SA_CONFIG.AGGREATION_CHANNEL[k])

        skip_channel_list = [1, 64, 128, 256, 512]
        channel_out = 512
        
        self.num_point_features = self.model_cfg.SA_CONFIG.FP_MLPS[0][-1]
    ...
    def forward(self, batch_dict):
        ...
        xyz = batch_dict['backbone_xyz'] #[B,N,3]
        features = batch_dict['backbone_features'] #[B,C,N]
        ...
        for i in range(len(self.SA_modules)):
            xyz_input = encoder_xyz[self.layer_inputs[i]] #3
            feature_input = encoder_features[self.layer_inputs[i]] #3
            if self.layer_types[i] == 'SA_Layer':
                ctr_xyz = None
                if self.ctr_indexes[i] != -1:
                    ctr_xyz = encoder_xyz[self.ctr_indexes[i]] #5
        ...
        # ctr_batch_idx = batch_idx.view(batch_size, -1)[:, :ctr_offsets.shape[1]] #[B,512]
        # ctr_batch_idx = ctr_batch_idx.contiguous().view(-1) #B*512
        ...
        return centers_origin, centers, encoder_features[-1]
